...
class RoomBloc extends Bloc<RoomEvent, RoomState> {
  RoomBloc() : super(const RoomState(roomOverviewState: RoomOverviewState.initial, player: null, room: null)) {
  ...
  void _onCreateClicked(
    CreateClicked event, Emitter<RoomState> emit) {
  ...
  void _onJoinSubmitted(JoinSubmitted event, Emitter<RoomState> emit) {
    emit(state.copyWith(roomOverviewState: RoomOverviewState.loading));
    emit(state.copyWith(roomOverviewState: RoomOverviewState.active, room: event.room, player: event.player));
  }
